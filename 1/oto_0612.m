...
for i = 1:1
    ...
    % グラフの描画
    figure
    plot(f_AD, power_AD, 'LineWidth', 0.5, 'Color', "blue");
    xlabel('Frequency [Hz]');
    ylabel('Power');
    title('Power Spectrum (AD\_SSW)');

    % スペクトルピーク検出パラメータ
    num_peaks = 3;  % 検出するピークの数

    % スペクトルピーク検出
    [~, peak_indices] = findpeaks(P1_AD, 'SortStr', 'descend', 'NPeaks', num_peaks);

    % フォルマント周波数の取得
    formant_freqs = f_AD(peak_indices);

    % 結果の表示
    disp('Formant Frequencies:');
    disp(formant_freqs);
